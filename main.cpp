...
class Class1 { //класс, куда помещаем образы и разбиваем по классам (нет блин по приколам)
public:
    ...
    float final; // корень из (x-y)^T * (S+E)^(-1) * (x-y)
    Class1() {};
    ~Class1() {};
};
...
void E_M(vector<vector<float>>& M) {
    ...
    for (int i = 0; i < M.size(); i++)
        ...
        {
            if (i == j) { E[i][j] = 1.0; }
            M[i][j] += E[i][j]; //сразу добавляем в матрицу ковариации
        }
    ...
    for (int i = 0; i < M.size(); i++) {
        for (int b = 0; b < M.size(); b++) {
            M[i][b] = E[i][b]; //ответ будет в единичной матрице и ее надр будет переписать в матрицу ковариацции
        }
    }
}
...
int main(int argc, char* argv[]) //argc - argumnet counter, argv - argument values
{
    ...
    for (int step = 0;step < clauses_amount;step++) {
        ...
        for (int i = 0;i < n;i++) {
            for (int j = 0;j < m;j++) {
                for (int obr = 0;obr < image_amount;obr++) {
                    oblast[step][0].core[i][j] += oblast[step][obr].coord[i][j]; //суммируем наши координаты
                } //для каждой координаты суммируем координаты всех образов класса
                oblast[step][0].core[i][j] /= (n * m); //делим на размерность изображения
    ...
    for (int st = 0; st < task_image_amount; st++) {
        for (int i = 0;i < n;i++) {
            for (int j = 0;j < m;j++) {
                for (int step = 0;step < clauses_amount;step++) {
                    oblast[step][0].rasnost_coord[i][j] = loader.tasks[st][i][j] - oblast[step][0].core[i][j]; //обнуляем матрицу, в кот будет хранится произведение (x-y)^T * обр мат ков
                }
            }
        }
        ...
        vector<pair<float, int>> best; //подсчитывем дистанцию
        for (int step = 0;step < clauses_amount;step++) {
        ...
        loader.print_tasks(st, loader.classes[best[0].second * 4]); //st -порядковый номер картинки, передаем наим знач дистанции опр класса
        for (int step = 0;step < clauses_amount;step++) { //вводим дистанцию
